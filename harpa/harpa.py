...
from harpa.utils import annotation
from harpa.sgld import SGLD
from harpa.model import AssignmentModel, AssignmentModel_NF
...
def preprocess_data(picks=None, station_df=None, config=None):
    ...
    if "time_before" in config:
        ...
        time_before = (
            (config["x(km)"][1] - config["x(km)"][0]) ** 2
            + (config["y(km)"][1] - config["y(km)"][0]) ** 2
            + (config["z(km)"][1] - config["z(km)"][0]) ** 2
        ) ** 0.5 / config["vel"]["P"]
    ...
    config["x(km)"] = (
        config["x(km)"][0]
        - (config["x(km)"][1] - config["x(km)"][0]) * config["boundary_rate"],
        config["x(km)"][1]
        + (config["x(km)"][1] - config["x(km)"][0]) * config["boundary_rate"],
    )
    config["y(km)"] = (
        config["y(km)"][0]
        - (config["y(km)"][1] - config["y(km)"][0]) * config["boundary_rate"],
        config["y(km)"][1]
        + (config["y(km)"][1] - config["y(km)"][0]) * config["boundary_rate"],
    )
    config["z(km)"] = (
        config["z(km)"][0]
        - (config["z(km)"][1] - config["z(km)"][0]) * config["boundary_rate"],
        config["z(km)"][1]
        + (config["z(km)"][1] - config["z(km)"][0]) * config["boundary_rate"],
    )
    ...
    if config["P_phase"]:
        for idx, station in station_df.iterrows():
            ...
            arrival_time_list = pick_df["time_ref"][
                (pick_df.station_id == station_id) & (pick_df.type == "P")
            ]
            arrival_pickindex_list = pick_df.index[
                (pick_df.station_id == station_id) & (pick_df.type == "P")
            ]
            ...
            station_df.at[idx, "arrival_time_list_P"] = arrival_time_list[
                sorted_indices
            ].tolist()
            station_df.at[idx, "arrival_pickindex_list_P"] = arrival_pickindex_list[
                sorted_indices
            ].tolist()
    ...
    if config["S_phase"]:
        for idx, station in station_df.iterrows():
            station_id = station.station_id
            arrival_time_list = pick_df["time_ref"][
                (pick_df.station_id == station_id) & (pick_df.type == "S")
            ]
            arrival_pickindex_list = pick_df.index[
                (pick_df.station_id == station_id) & (pick_df.type == "S")
            ]
            ...
            arrival_pickindex_list = np.array(arrival_pickindex_list.values)
            sorted_indices = np.argsort(arrival_time_list)
            station_df.at[idx, "arrival_time_list_S"] = arrival_time_list[
                sorted_indices
            ].tolist()
            station_df.at[idx, "arrival_pickindex_list_S"] = arrival_pickindex_list[
                sorted_indices
            ].tolist()
...
def association(picks, station_df, config, verbose=0, model_traveltime=None):
    ...
    default_config = {
        ...
        "patience_max": 10,
        "lr_decay": 0.1,
        "epoch_before_decay": 3000,
        "epoch_after_decay": 3000,
    ...
    if "ncpu" not in config:
        config["ncpu"] = max(
            1, min(len(unique_labels) // 4, min(100, mp.cpu_count() - 1))
        )
    else:
        config["ncpu"] = min(mp.cpu_count(), config["ncpu"])
    if verbose > 0:
        print(
            f"Associating {len(picks)} picks separated into {len(unique_labels)} slides with {config['ncpu']} CPUs"
        )
    ...
    if config["neural_field"]:
        ...
        for slice_index in tqdm(range(len(unique_labels))):
            pick_df, catalog_df, z = run_harpa(
                picks=picks[picks["labels"] == slice_index],
                station_df=station_df,
                config=config,
                verbose=verbose,
                skip=slice_index < 0,
                model_traveltime=model_traveltime,
            )
    ...
    pick_df, catalog_df = reindex_picks_and_events(
        pick_df_list, catalog_df_list, config, overlap=0, verbose=verbose
    )
    if verbose > 0:
...
def run_harpa(
    ...
    pick_df, station_df, config = preprocess_data(
        picks=picks, station_df=station_df, config=config
    )
    ...
    if config["n_event_max"] is None:
        n_event_max = 0
        pick_station_id = pick_df.apply(
            lambda x: x.station_id + "_" + x.type, axis=1
        ).to_numpy()
    ...
    if verbose > 6:
        print(
            "max number of event in this slice:",
            n_event_max,
            "number of picks in this slice:",
            len(pick_df),
        )
    ...
    if config["neural_field"]:
        if isinstance(model_traveltime, str):
            ...
            first_omega_0 = 30
            hidden_omega_0 = 30
            model_traveltime_cpu = Siren(
                in_features=3 + L,
                out_features=len(station_df),
                hidden_features=128,
                hidden_layers=3,
    ...
    if "init" in config:
        if config["init"] == "data":
            ...
            xs = meta["x(km)"].values
            ys = meta["y(km)"].values
            index = np.argsort(times)[:: max(len(times) // n_event_max, 1)][
                :n_event_max
            ]
            t_init = times[index]
            x_init = xs[index]
            ...
            x_init = (x_init - config["x(km)"][0]) / (
                config["x(km)"][1] - config["x(km)"][0]
            )
            y_init = (y_init - config["y(km)"][0]) / (
                config["y(km)"][1] - config["y(km)"][0]
            )
            z_init = (z_init - config["z(km)"][0]) / (
                config["z(km)"][1] - config["z(km)"][0]
            )
            t_init = (t_init - config["t(s)"][0]) / (
                config["t(s)"][1] - config["t(s)"][0]
            )
    ...
    for step in range(epoch_before_decay + epoch_after_decay):
        ...
        if step == epoch_after_decay:
            ...
            optimizer = SGLD(
                Harpa.parameters(), lr=lr * lr_decay, noise=noise * lr_decay
            )
        if len(true_time_list) > 0:
            ...
            loss, _ = compute_assignment_loss(
                time_list,
                true_time_list,
    ...
    if config["second_adjust"]:
        ...
        for training_index in range(n_event_max):
            event_picks_index_P = pick_df.index[
                (pick_df["event_index"] == training_index) & (pick_df["type"] == "P")
            ]
            event_picks_index_S = pick_df.index[
                (pick_df["event_index"] == training_index) & (pick_df["type"] == "S")
            ]
            ...
            if len(event_picks_index_P) + len(event_picks_index_S) > 0:
                ...
                pick_times_list_P = torch.tensor(
                    pick_df["time_ref"][event_picks_index_P].values
                ).to(device)
                pick_times_list_S = torch.tensor(
                    pick_df["time_ref"][event_picks_index_S].values
                ).to(device)

                id_list_P = pick_df["station_id"][event_picks_index_P]
                id_list_S = pick_df["station_id"][event_picks_index_S]
                event_picks_index_station_index_P = [
                    station_df.index[station_df["station_id"] == id_value].tolist()[0]
                    for id_value in id_list_P
                ]
                event_picks_index_station_index_S = [
                    station_df.index[station_df["station_id"] == id_value].tolist()[0]
                    for id_value in id_list_S
                ]
                lastloss = 100000
                for step in range(100):
                    ...
                    loss = 0
                    if config["P_phase"]:
                        for i, station_index in enumerate(
                            event_picks_index_station_index_P
                        ):
                            arrival_time_P = Harpa.arrival_time(
                                station_index, phase="P"
                            )[training_index]
                    ...
                    if config["S_phase"]:
                        for i, station_index in enumerate(
                            ...
                            arrival_time_S = Harpa.arrival_time(
                                station_index, phase="S"
                            )[training_index]
                            loss += (pick_times_list_S[i] - arrival_time_S) ** 2
                    if step % 10 == 0:
                        if verbose > 8:
                            print(
                                f"Training index: {training_index}, Step: {step}, Loss: {loss.item():.2f}"
                            )
    ...
    if config["denoise_rate"] > 0 and len(catalog_df) > 0:
        ...
        pick_df, catalog_df = denoise_events(
            pick_df,
            catalog_df,
            station_df,
            km=True,
            MIN_NEAREST_STATION_RATIO=config["denoise_rate"],
            verbose=verbose,
        )
    if "time_ref" in pick_df.columns:
        pick_df = pick_df.drop(columns=["time_ref"])
