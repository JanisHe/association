...
def plot_assosication(
    pick_df, event_idx_key, station_df, catalog, plot_y_as="x", xlim=None, plot_max=100
):
    station_dict = {
        station: (x, y)
        for station, x, y in zip(
            station_df["station_id"], station_df["x(km)"], station_df["y(km)"]
        )
    }
    ...
    if plot_y_as == "x":
        pick_x_list = [
            station_dict[pick_id][0] for pick_id in pick_df["station_id"].values
        ]
        pick_x_list = np.array(pick_x_list)
        y_axis = pick_x_list
    if plot_y_as == "y":
        pick_y_list = [
            station_dict[pick_id][1] for pick_id in pick_df["station_id"].values
        ]
    ...
    ax.scatter(
        d_time_idx_list[plot_mask],
        y_axis[plot_mask],
        s=20,
        color=colors[event_idx_list[plot_mask]],
    )
...
def plot_line(
    ...
    for idx, event in catalog.iterrows():
        ...
        for p, phase in enumerate(phases):
            ...
            pick_index_list = pick_df.index[
                (pick_df[event_idx_key] == event.event_index)
                & (pick_df["type"] == phase)
            ]
            ...
            if len(pick_time) > 0:
                ...
                station_x = np.array(
                    [
                        station_dict[sid][0]
                        for sid in pick_df.station_id[pick_index_list].values
                    ]
                )
                station_y = np.array(
                    [
                ...
                ax.scatter(
                    ...
                    dist,
                    s=20,
                    color=colors[event.event_index],
                    marker=markers[p],
                )
            ax.text(
...
def plot_assosication_time_window(
    pick_df,
    event_idx_key,
    station_df,
    catalog,
    ...
    if event_idx_mask.sum() > 0:
        ax.scatter(
            time_idx_list[event_idx_mask],
            y_axis[event_idx_mask],
            s=20,
            color=colors[(event_idx_list[event_idx_mask] % 10)],
        )
    if (~event_idx_mask).sum() > 0:
        ax.scatter(
            time_idx_list[~event_idx_mask], y_axis[~event_idx_mask], s=20, c="black"
        )
    ...
    ax.set_xlim(
        [catalog.time.min() - pd.Timedelta(seconds=1), pick_df["timestamp"].max()]
    )

    return ax


def plot_assosication_line_window(
    pick_df,
    event_idx_key,
    station_df,
    catalog,
        ...
        for p, phase in enumerate(phases):
            ...
            if len(pick_time) > 0:
                ...
                ax.scatter(
                    ...
                    dist,
                    s=20,
                    color=colors[event.event_index % 10],
                    marker=markers[p],
                )
            ax.text(
                event.time,
            ...
            ax.scatter(
                event.time, 0, marker="o", s=40, color=colors[event.event_index % 10]
            )
    for p, phase in enumerate(phases):
...
def plot3D(V, X, vmin=None, vmax=None, colorbar=True, show=True):
    ...
    if colorbar:
        ...
        fig.data[0].colorbar.tickfont = dict(family="Helvetica", color="black", size=20)
    ...
    camera = dict(
        eye=dict(
            x=0.9764760583095522 * r, y=1.7327485214077152 * r, z=0.6319680639439241 * r
        ),
    ...
    fig.update_layout(font=dict(family="Helvetica", color="black", size=16))
...
def plot_assosication_window(
    ...
    for idx, event in catalog.iterrows():
        for p, phase in enumerate(phases):
            ...
            pick_time = pick_df.timestamp[pick_index_list].values
            ax.scatter(
                event.time, 0, marker="D", s=40, color=colors[event.event_index % 10]
            )
    ...
    ax.set_xlim(
        [
            catalog.time.min() - pd.Timedelta(seconds=4),
            pick_df["timestamp"].max() + pd.Timedelta(seconds=5),
        ]
    )
    ...
    if (~event_idx_mask).sum() > 0:
        ax.scatter(
            time_idx_list[~event_idx_mask], y_axis[~event_idx_mask], s=s, c="black"
        )
    ...
    for event in catalog.iloc:
        ...
        ax.plot(
            [x, x],
            [current_ylim[0] - 10, current_ylim[1] + 10],
            color=colors[int(event.event_index) % 10],
        )
