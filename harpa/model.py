...
class AssignmentModel(nn.Module):
    def __init__(self, config, station_df, seed=0, device="cpu"):
        ...
        self.event_times = nn.Parameter(
            torch.logit(torch.linspace(0.05, 0.95, steps=n_event))
        )
    ...
    def loc_and_time(self):
        ...
        # print(event_times)
        event_locsx = (
            (event_locs[:, 0]) * (self.config["x(km)"][1] - self.config["x(km)"][0])
            + self.config["x(km)"][0]
        )
        event_locsy = (
            (event_locs[:, 1]) * (self.config["y(km)"][1] - self.config["y(km)"][0])
            + self.config["y(km)"][0]
        )
        event_locsz = (
            (event_locs[:, 2]) * (self.config["z(km)"][1] - self.config["z(km)"][0])
            + self.config["z(km)"][0]
        )
        event_locs = torch.vstack([event_locsx, event_locsy, event_locsz]).T
        event_times = (
            event_times * (self.config["t(s)"][1] - self.config["t(s)"][0])
            + self.config["t(s)"][0]
        )
...
class AssignmentModel_NF(nn.Module):
    def __init__(self, config, station_df, seed=0, device="cpu", model_traveltime=None):
        ...
        self.event_locs.requires_grad_(True)
        self.event_times = nn.Parameter(
            torch.logit(torch.linspace(0.05, 0.95, steps=n_event)).float()
        )
        self.event_times.requires_grad_(True)
        self.config = config
        self.station_df = station_df
...
class Encoder(nn.Module):
    def __init__(self, capacity=8, latent_dims=2):
        ...
        self.conv1 = nn.Conv3d(
            in_channels=1, out_channels=c, kernel_size=3, stride=2, padding=1
        )
        self.bn1 = nn.BatchNorm3d(c)  #
        self.conv2 = nn.Conv3d(
            in_channels=c, out_channels=c * 2, kernel_size=3, stride=2, padding=1
        )
        self.bn2 = nn.BatchNorm3d(2 * c)
        self.conv3 = nn.Conv3d(
            in_channels=c * 2, out_channels=c * 4, kernel_size=3, stride=2, padding=1
        )
...
class Decoder(nn.Module):
    def __init__(
        self, capacity=8, latent_dims=4
    ):  # latent_dims=4 as per your clarification
        super(Decoder, self).__init__()
        c = capacity
        self.fc = nn.Linear(
            in_features=latent_dims, out_features=4 * c * 4 * 4 * 4
        )  # latent vector mapped to 4x4x4 feature map
        self.bn3 = nn.BatchNorm3d(4 * c)
        ...
        self.conv2 = nn.ConvTranspose3d(
            in_channels=2 * c, out_channels=c, kernel_size=4, stride=2, padding=2
        )
        self.bn1 = nn.BatchNorm3d(c)
        self.conv1 = nn.ConvTranspose3d(
            in_channels=c, out_channels=1, kernel_size=4, stride=2, padding=2
        )  # Adjusted for 26x26x26 output
    ...
    def forward(self, x):
        x = self.fc(x)
        x = F.relu(
            self.bn3(x.view(x.size(0), -1, 4, 4, 4))
