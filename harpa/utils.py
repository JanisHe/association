...
def compute_assignment_loss(list_a, list_b, noisy_pick=True, LSA=True, p=2):
    if LSA:
        ...
        row_ind, col_ind = linear_sum_assignment(
            distance_matrix.clone().detach().cpu().numpy()
        )
...
def annotation(
    ...
    min_peak_pre_event,
    start_time_ref,
    sort_evnet_index=False,
    phases=["P"],
    min_peak_pre_event_p=-1,
    min_peak_pre_event_s=-1,
):
    ...
    for phase in phases:
        for station_index in range(n_station):
            ...
            pickindex_list = station_df["arrival_pickindex_list_" + phase][
                station_index
            ]
    ...
    event_locs, event_times = (
        event_locs.detach().clone().cpu().numpy(),
        event_times.detach().clone().cpu().numpy(),
    )
    catalog_df = []
    old_event_idx_list = []
    new_event_idx_list = []
    for event_index in range(n_event):
        ...
        num_p_picks = pick_df["station_id"][
            (pick_df.event_index == event_index) & (pick_df.type == "P")
        ].nunique()
        num_s_picks = pick_df["station_id"][
            (pick_df.event_index == event_index) & (pick_df.type == "S")
        ].nunique()
...
def denoise_events(
    pick_df, catalog_df, station_df, km=True, MIN_NEAREST_STATION_RATIO=0.3, verbose=2
):
    ...
    picks = picks.merge(
        events[["event_index", longitude, latitude]],
        on="event_index",
        suffixes=("", "_event"),
    )
    picks = picks.merge(
        stations[["station_id", longitude, latitude]],
        on="station_id",
        suffixes=("", "_station"),
    )
...
def catalog_interval(catalog_df, interval_number=10):
    ...
    for i in range(len(catalog_df)):
        if i - half_interval <= 0:
            ...
            inter = (
                time_seconds[-1] - time_seconds[-1 - interval_number]
            ) / interval_number
        else:
            inter = (
                time_seconds[i + half_interval] - time_seconds[i - half_interval]
            ) / interval_number
        # if inter<0:
        #     print(i)
...
def merge_catalogs(
    ...
    def process_catalog_inputs(catalog_out_df, catalog_df_next):
        if not catalog_df_next[
            catalog_df_next["event_index"].isin(catalog_out_df["event_index"])
        ].empty:
            # Merge those events in both catalog_out_df and catalog_df_next
            agg_dict = {
                col: "first"
                for col in catalog_out_df.columns
                if col not in ["num_p_picks", "num_s_picks", "num_picks"]
            }
            ...
            catalog_out_df = (
                pd.concat(
                    [
                        catalog_out_df,
                        catalog_df_next[
                            catalog_df_next["event_index"].isin(
                                catalog_out_df["event_index"]
                            )
                        ],
                    ]
                )
            ...
            catalog_df_next = catalog_df_next[
                ~catalog_df_next["event_index"].isin(catalog_out_df["event_index"])
            ]
        return catalog_out_df, catalog_df_next

    catalog_out_df, catalog_df_next = process_catalog_inputs(
        catalog_out_df, catalog_df_next
    )
    # define overlap catalog
    catalog_overlap = catalog_out_df[
        (
            catalog_out_df["time"]
            > catalog_df_next["time"].min() - pd.Timedelta(seconds=threshold_time)
        )
        & (
            catalog_out_df["time"]
            < catalog_df_next["time"].max() + pd.Timedelta(seconds=threshold_time)
        )
    ]
    ...
    time_diff = np.abs(
        catalog_overlap["time"].values[:, None] - catalog_df_next["time"].values
    )
    ...
    for i, j in zip(*np.where(valid_pairs)):
        ...
        pick_df_next.loc[
            pick_df_next["event_index"] == catalog_event_index, "event_index"
        ] = event_index_to_update

    # Convert merged rows into a DataFrame
    merged_df = pd.DataFrame(merged_rows)
    # Remove merged rows from the original dataframes
    catalog_overlap = catalog_overlap.drop(
        catalog_overlap.index[np.where(valid_pairs)[0]]
    )
    catalog_df_next = catalog_df_next.drop(
        catalog_df_next.index[np.where(valid_pairs)[1]]
    )
    # Concatenate the merged rows and remaining rows
    catalog_out_df = pd.concat(
        [catalog_overlap, merged_df, catalog_non_overlap, catalog_df_next]
    )
...
def get_index_update_map_from_duplicates(pick_df_next, pick_df_overlap_eventpick):
    ...
    merged_df = pd.merge(
        pick_df_next.set_index(index_cols),
        pick_df_overlap_eventpick.set_index(index_cols),
        suffixes=("_next", "_overlap"),
        how="inner",
        left_index=True,
        right_index=True,
    )  # with inner join, only duplicate indices are kept
    # Use groupby to count the number of matching event index pairs
    comparison_counts = merged_df.groupby(
        ["event_index_next", "event_index_overlap"]
    ).size()
    ...
    dominant_indices = comparison_percentages[
        comparison_percentages > THRESHOLD_OF_DUPLICATED_INDEX_MATCH
    ]
...
def reindex_picks_and_events(
    pick_df_list, catalog_df_list, harpa_config, overlap=None, verbose=1000
):
    def merge_pick_df(
        pick_df_overlap,
        pick_df_next,
        pick_out_df,
        pick_df_next_first_event_time,
        pick_df_next_last_event_time,
    ):
        import pandas as pd

        # Merge overlapping picks and remove duplicates
        pick_merged_df = pd.merge(
            pick_df_overlap,
            pick_df_next,
            left_index=True,
            right_index=True,
            how="outer",
            suffixes=("_overlap", "_next"),
        )
        mask = (pick_merged_df["event_index_overlap"] == -1) | (
            pick_merged_df["event_index_overlap"].isna()
        )
        for column in pick_df_overlap.columns:
            ...
            pick_merged_df[column] = np.where(
                condition_met,
                pick_merged_df[f"{column}_next"],
                pick_merged_df[f"{column}_overlap"],
            )
        columns_to_drop = [
            col
            for col in pick_merged_df.columns
            if col.endswith("_overlap") or col.endswith("_next")
        ]
    ...
    for pick_df, catalog_df in tqdm(
        zip(pick_df_list, catalog_df_list), total=len(pick_df_list)
    ):
        ...
        if not isinstance(pick_out_df, pd.DataFrame):
            ...
            if overlap > 0:
                ...
                pick_df_overlap = pick_out_df[
                    (
                        pick_out_df["timestamp"]
                        > pick_df_next_first_event_time - pd.Timedelta(seconds=overlap)
                    )
                    & (
                        pick_out_df["timestamp"]
                        < pick_df_next_last_event_time + pd.Timedelta(seconds=overlap)
                    )
                ]

                # Filter out picks that are duplicates based on station_id, timestamp, and type
                pick_df_overlap_eventpick = pick_df_overlap[
                    pick_df_overlap["event_index"] > -1
                ]
                # analyze duplicated picks to find match of event_index
                non_duplicate_df = pick_df_next[
                    ~pick_df_next.set_index(
                        ["station_id", "timestamp", "type"]
                    ).index.isin(
                        pick_df_overlap_eventpick.set_index(
                            ["station_id", "timestamp", "type"]
                        ).index
                    )
                ].copy()
                index_update_map = get_index_update_map_from_duplicates(
                    pick_df_next, pick_df_overlap_eventpick
                )
                ...
                next_event_index_list = event_counts.index[
                    (event_counts >= min_peak_pre_event)
                ]
                ...
                for index in catalog_df_next.index.values:
                    if (
                        index in index_update_map
                        and pick_df_next[pick_df_next["event_index"] == index].shape[0]
                        >= min_peak_pre_event
                    ):
                        ...
                        index in next_event_index_list
                        and non_duplicate_df[
                            non_duplicate_df["event_index"] == index
                        ].shape[0]
                        >= min_peak_pre_event
                    ):
                ...
                replace_dict = dict(zip(old_event_idx_list, new_event_idx_list))
                pick_df_next["event_index"] = pick_df_next["event_index"].replace(
                    replace_dict
                )

                # Count the number of P-type/S-type picks associated with each event
                num_p_picks = (
                    non_duplicate_df[non_duplicate_df["type"] == "P"]
                    .groupby("event_index")
                    .size()
                )
                num_s_picks = (
                    non_duplicate_df[non_duplicate_df["type"] == "S"]
                    .groupby("event_index")
                    .size()
                )
                catalog_df_next["num_p_picks"] = (
                    catalog_df_next["event_index"]
                    .map(num_p_picks)
                    .fillna(0)
                    .astype(int)
                )
                catalog_df_next["num_s_picks"] = (
                    catalog_df_next["event_index"]
                    .map(num_s_picks)
                    .fillna(0)
                    .astype(int)
                )
                catalog_df_next["num_picks"] = (
                    (catalog_df_next["num_p_picks"] + catalog_df_next["num_s_picks"])
                    .fillna(0)
                    .astype(int)
                )
                catalog_df_next["event_index"] = catalog_df_next["event_index"].replace(
                    replace_dict
                )
                ...
                if catalog_df_next.empty:
                    pick_out_df = merge_pick_df(
                        pick_df_overlap,
                        pick_df_next,
                        pick_out_df,
                        pick_df_next_first_event_time,
                        pick_df_next_last_event_time,
                    )  # merge pick_out_df with next and continue
                    current_index = (
                        catalog_out_df.event_index.max() + 1
                        if not catalog_out_df.empty
                        else 0
                    )
                    continue
                ...
                if catalog_out_df.empty:
                    ...
                    catalog_out_df, pick_df_next = merge_catalogs(
                        catalog_out_df,
                        catalog_df_next,
                        pick_df_next,
                        threshold_time,
                    ...
                    for col in catalog_out_df.columns:
                        if col in ["num_p_picks", "num_s_picks", "num_picks"]:
                            ...
                            agg_dict[col] = "first"
                    catalog_out_df = catalog_out_df.groupby(
                        "event_index", as_index=False
                    ).agg(agg_dict)
                # Merge overlapping picks and remove duplicates
                pick_out_df = merge_pick_df(
                    pick_df_overlap,
                ...
                pick_df_next = pick_df.copy()
                pick_df_next.loc[pick_df_next["event_index"] > -1, "event_index"] += (
                    current_index
                )
    ...
    if verbose > 2:
        print(
            f"removed {n_repeat_event} repeated events, left {len(catalog_out_df)} unique events"
        )
    # if overlap > 0:
    ...
    pick_out_df["event_index"] = (
        pick_out_df["event_index"].map(mapping).fillna(-1).astype(int)
    )

    return pick_out_df, catalog_out_df


def reindex_picks_and_events_fast(
    pick_df_list, catalog_df_list, harpa_config, overlap=None, verbose=1000
):
    # Print the status message
    if verbose > 2:
...
def compute_mag(pick_df, catalog_df, station_df):
    ...
    merged_df = pd.merge(
        merged_df, station_df, on="station_id", how="left", suffixes=("", "_station")
    )
    ...
    merged_df.loc[event_mask, "distance"] = np.sqrt(
        (
            merged_df.loc[event_mask, "x(km)"]
            - merged_df.loc[event_mask, "x(km)_station"]
        )
        ** 2
        + (
            merged_df.loc[event_mask, "y(km)"]
            - merged_df.loc[event_mask, "y(km)_station"]
        )
        ** 2
        + (
            merged_df.loc[event_mask, "z(km)"]
            - merged_df.loc[event_mask, "z(km)_station"]
        )
        ** 2
    )
...
def check_and_clean_event_index(
    pick_df, catalog_df, correction=True, stations=None, column="event_index"
):
    """
    ...
    if correction:
        ...
        merged_df["new_time"] = (
            np.nan
        )  # Using np.nan instead of None for numerical operations
        ...
        merged_df.loc[condition_P, "new_time"] = merged_df["time"][
            condition_P
        ] + pd.to_timedelta(merged_df["distance"][condition_P] / 6, unit="s")
        ...
        merged_df.loc[condition_S, "new_time"] = merged_df["time"][
            condition_S
        ] + pd.to_timedelta(merged_df["distance"][condition_S] / (6 / 1.75), unit="s")
...
def compress_data(pick_df, catalog_df, rate=1, shuffle=False):
    ...
    if shuffle:
        ...
        for i, event in catalog_df.iterrows():
            ...
            pick_df.loc[pick_df["event_index"] == event_index, "timestamp"] += (
                pd.Timedelta(seconds=time_diff)
            )
    ...
    for i, event in catalog_df.iterrows():
        event_index = event.event_index
        time_diff = (event["time"] - event["time_old"]).total_seconds()
        pick_df.loc[pick_df["event_index"] == event_index, "timestamp"] += pd.Timedelta(
            seconds=time_diff
        )
    return pick_df, catalog_df
...
def remove_repeat_events(picks, catalog_df, time_threshold=5, space_threshold=10):
    ...
    evaluation_matrix_time = (
        np.abs(t_pred[np.newaxis, :] - t_pred[:, np.newaxis]) < time_threshold
    )
    ...
    true_coords = np.column_stack(upper_triangle_indices)[
        evaluation_matrix[upper_triangle_indices]
    ]

    picks_ = picks.copy()
    catalog_df_ = catalog_df.copy()

    picks_["station_id_phase"] = picks_.apply(
        lambda x: x.station_id + "_" + x.phase_type, axis=1
    ).to_numpy()

    for x_i, y_i in true_coords:
        x_label = catalog_df_["event_index"].iloc[x_i]
        y_label = catalog_df_["event_index"].iloc[y_i]
        x_station_id_phase = picks_["station_id_phase"][
            picks_["event_index"] == x_label
        ]
        y_station_id_phase = picks_["station_id_phase"][
            picks_["event_index"] == y_label
        ]
    ...
    print(
        f"removed {len(catalog_df) - len(catalog_df_)} repeated events, left {len(catalog_df_)} unique events"
    )
