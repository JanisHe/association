...
def load_ridgecrest(time_start=None, time_end=None):
    ...
    data_path = lambda x: os.path.join("./Data/Ridgecrest/test_data", x)
    region = "ridgecrest"
...
def load_ridgecrest_full(time_start=None, time_end=None, drop_index=True):
    data_path = lambda x: os.path.join("../Data/Ridgecrest", x)
    region = "ridgecrest"
    ...
    stations.rename(
        columns={"station": "id", "elevation(m)": "elevation_m"}, inplace=True
    )
    # print("Station format:", stations.iloc[:10])
...
def load_chile(time_start=None, time_end=None):
    data_path = lambda x: os.path.join("../Data/Chile/test_data", x)
    region = "chile"
    station_csv = data_path("stations.csv")
    picks_csv = data_path("picks.csv")
    picks = pd.read_csv(picks_csv, parse_dates=["phase_time"])
    ...
    velocity_model = pd.read_csv(
        data_path("iasp91.csv"), names=["zz", "rho", "vp", "vs"]
    )
