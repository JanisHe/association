...
def get_traveling_time_multi(V_list, station_df, config, dx=25, align_station=False):
    ...
    Z = Z.transpose(1, 0, 2)
    if align_station:
        station_df["x(km)"] = station_df["x(km)"].apply(
            lambda x: xx[np.argmin(np.abs(np.array(xx) - x))]
        )
        station_df["y(km)"] = station_df["y(km)"].apply(
            lambda x: yy[np.argmin(np.abs(np.array(yy) - x))]
        )
        station_df["z(km)"] = station_df["z(km)"].apply(
            lambda x: zz[np.argmin(np.abs(np.array(zz) - x))]
        )

    station_locs = np.array(
        [station_df["x(km)"], station_df["y(km)"], station_df["z(km)"]]
    ).T
    ...
    travel_time = np.zeros(
        (n_wavespeed, n_station, grid_size_x, grid_size_y, grid_size_z)
    )
...
def get_traveling_time_multi_mp(V_list, station_df, config, dx=25, align_station=False):
    ...
    result_list = (
        ...
        .astype(np.float16)
        .reshape(n_wavespeed, n_station, grid_size_x, grid_size_y, grid_size_z)
    )
    return result_list
...
def gen_source(n_source, frequence, config, seed=0, randomtime=False):
    ...
    for n in range(n_source):
        ...
        idxs = [
            np.random.randint(1, len(x) - 1),
            np.random.randint(1, len(y) - 1),
            np.random.randint(11, len(z) - 1),
        ]  # avoid boundary
...
class TravelTimeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        idx, id_ = (
            id_ // (self.grid_sizes[1] * self.grid_sizes[2]),
            id_ % (self.grid_sizes[1] * self.grid_sizes[2]),
        )
        idy, idz = id_ // self.grid_sizes[2], id_ % self.grid_sizes[2]
        input = np.concatenate(
            [
                self.V_paramter_list[idx_wavespeed],
                [self.X[idx, idy, idz]],
                [self.Y[idx, idy, idz]],
                [self.Z[idx, idy, idz]],
            ]
...
class TravelTimeDataset_NF(Dataset):
    def __init__(
        ...
        with torch.no_grad():
            data = (
                torch.tensor(WaveSpeedData.data_wavespeed)
                .unsqueeze(1)
                .float()
                .to(device)
            )
