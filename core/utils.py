...
def area_limits(stations: pd.DataFrame,
                lat_lon_eps: float = 0.2) -> dict:
    limits = {
        "latitude": (min(stations["latitude"]) - lat_lon_eps, max(stations["latitude"]) + lat_lon_eps),
        "longitude": (min(stations["longitude"]) - lat_lon_eps, max(stations["longitude"]) + lat_lon_eps)
    }
