...
from association.core.utils import sort_events, area_limits
...
class Event2NLL:
    ...
    def create_nll_dirs(self, delete_basepath: bool = True):
        """
    ...
    def create_obs(self, delete_file: bool = False):
        """
        ...
        if delete_file:
            try:
                ...
                os.remove(
                    os.path.join(self.nll_basepath, "obs", "events.obs")
                )  # Delete existing .obs file
    ...
    def create_config(
        ...
        # Write config file
        with open(
            os.path.join(self.nll_basepath, "conf", f"{self.config_name}.conf"), "w"
        ) as f_conf:
            ...
            f_conf.write(
                f"VGOUT {os.path.join(self.nll_basepath, 'model', self.config_name)}\n"
            )
            ...
            for ind in range(len(self.df_vel_model)):
                f_conf.write(
                    f"LAYER {self.df_vel_model['depth'][ind]} {self.df_vel_model['vp'][ind]} 0 "
            ...
            f_conf.write(
                f"GTFILES {os.path.join(self.nll_basepath, 'model', self.config_name)} "
            ...
            for index in range(len(self.df_stations)):
                f_conf.write(
                    f"GTSRCE {self.df_stations['id'][index].split('.')[1]} LATLON "
            ...
            f_conf.write(
                f"LOCFILES {os.path.join(self.nll_basepath, 'obs', '*.obs')} NLLOC_OBS "
            ...
            f_conf.write(
                "LOCGRID 6000 6000 1400 -200.0 -250.0 0.0 0.1 0.1 0.1 PROB_DENSITY SAVE\n\n"
            )  # XXX
    ...
    def run_nll(self, delete_dirs: Optional[list] = None):
        """
    ...
    def delete_dir_content(self, directory: str):
        """
...
def update_events_from_nll(
    station_df: Union[str, pd.DataFrame], nll_basepath: str, depth_filter: float = 10000
) -> list:
    """
    ...
    for station_id in df_stations["id"]:
        ...
        for phase in ["P", "S"]:
            for ftype in ["angle.buf", "angle.hdr", "time.buf", "time.hdr"]:
                try:
                    filename = glob.glob(
                        os.path.join(nll_basepath, "time", f"*{phase}.{name}.{ftype}")
                    )[0]
                except IndexError:
                    return False
...
def nll_wrapper(
    ...
    nll_time_files_exist = check_nll_time(
        stations=station_df, nll_basepath=nll_basepath
    )
    ...
    if nll_time_files_exist is True:
        ...
        nll.delete_dir_content(
            directory=os.path.join(nll_basepath, "locs")
        )  # Delete locs directory
    ...
    all_events = update_events_from_nll(
        station_df=station_df, nll_basepath=nll_basepath
    )
